...
try {
  ...
  await emit('run.request',{request:3,directory,stages:[{program:'printf',resolved:'/usr/bin/printf',args:['hello'],display:'printf hello > /tmp/result.txt'},{program:'true',resolved:'/usr/bin/true',args:[],display:'true'}],plan:executionPlan,writes:['/tmp/result.txt'],releasesPrivate:false,vouches:[],summary:'run two steps'});

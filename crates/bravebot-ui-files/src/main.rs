...
fn open_at(parent: &File, part: &str, flags: i32) -> io::Result<File> {
    let part = name(part)?;
    // The caller owns the fd, and the CString remains alive for the syscall.
    let fd = unsafe {
        libc::openat(
            parent.as_raw_fd(),
            part.as_ptr(),
...
fn directory_at(parent: &File, part: &str, create: bool) -> io::Result<File> {
    if create {
        let part = name(part)?;
        let result = unsafe { libc::mkdirat(parent.as_raw_fd(), part.as_ptr(), 0o700) };
        if result < 0 && io::Error::last_os_error().kind() != io::ErrorKind::AlreadyExists {
            return Err(io::Error::last_os_error());
        }
    }
    open_at(parent, part, libc::O_RDONLY | libc::O_DIRECTORY)
}
...
fn read_at(parent: &File, leaf: &str, limit: usize) -> io::Result<(String, bool)> {
    let file = open_at(parent, leaf, libc::O_RDONLY)?;
...
mod tests {
    ...
    fn preview_stays_on_the_pinned_directory_when_its_parent_is_replaced() {
        ...
        fs::write(f.0.join("folder/file"), "inside").unwrap();
        fs::write(f.0.join("outside/file"), "private outside").unwrap();
        let (parent, leaf) = parent_directory(f.root(), "folder/file", false).unwrap();
        fs::rename(f.0.join("folder"), f.0.join("original")).unwrap();
        symlink(f.0.join("outside"), f.0.join("folder")).unwrap();
    ...
    fn memory_replace_cannot_follow_a_swapped_parent_to_an_outside_file() {
        ...
        fs::create_dir(f.0.join("outside")).unwrap();
        fs::write(f.0.join("outside/test.md"), "outside sentinel").unwrap();
        fs::rename(f.0.join(".bravebot-ui/bots"), f.0.join("original")).unwrap();
        symlink(f.0.join("outside"), f.0.join(".bravebot-ui/bots")).unwrap();
        replace_at(&parent, &leaf, "edited", Some("before")).unwrap();
        assert_eq!(
            fs::read_to_string(f.0.join("outside/test.md")).unwrap(),
            "outside sentinel"
        );
        assert_eq!(
            fs::read_to_string(f.0.join("original/test.md")).unwrap(),
            "edited"
        );
    ...
    fn a_symlink_replacing_the_root_or_leaf_is_refused() {
        ...
        fs::write(f.0.join("secret"), "outside").unwrap();
        symlink(f.0.join("secret"), f.0.join("project/file")).unwrap();
        ...
        symlink(&f.0, f.0.join("linked-root")).unwrap();
        assert!(root_directory(f.0.join("linked-root").to_str().unwrap()).is_err());
        assert_eq!(fs::read_to_string(f.0.join("secret")).unwrap(), "outside");
    }
    ...
    fn replacement_checks_expected_text_and_creates_a_private_regular_file() {
        ...
        assert_eq!(
            fs::metadata(f.0.join(".bravebot-ui/bots/test.md"))
                .unwrap()
                .permissions()
                .mode()
                & 0o777,
        ...
        assert_eq!(
            fs::read_dir(f.0.join(".bravebot-ui/bots")).unwrap().count(),
            1
        );
    }
    ...
    fn traversal_nonregular_binary_and_oversized_reads_are_bounded() {
        ...
        fs::write(f.0.join("binary"), b"a\0b").unwrap();
        fs::write(f.0.join("large"), "123456").unwrap();
